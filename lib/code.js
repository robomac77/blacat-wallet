                    ...
                    BlackCat.ViewTransferConfirm.callback = (params, net_fee) => __awaiter(this, void 0, void 0, function* () {
                        ...
                        setTimeout(() => __awaiter(this, void 0, void 0, function* () {
                            try {
                                ...
                                res['ext'] = e.toString();
                                console.log("[BlaCat]", '[main]', 'makeGasTransfer, ViewTransferConfirm.callback error, params => ', params, 'e => ', e.toString());
                            }
                            Main.viewMgr.viewLoading.remove();
                            var callback_data = {
                                params: params,
                            ...
                            if (Main.transGasCallback) {
                                try {
                                    ...
                                    console.log("[BlaCat]", '[main]', 'makeGasTransfer, app callback error! ViewTransferConfirm.callback error, params => ', params, 'e => ', e.toString());
                                }
                            }
...
(function (BlackCat) {
    class PayExchangeShowWalletView extends BlackCat.ViewBase {
        ...
        create() {
            ...
            refundButton.textContent = BlackCat.Main.langMgr.get("refund");
            this.ObjAppend(divAddress, refundButton);
            refundButton.onclick = () => {
                BlackCat.Main.viewMgr.change("RefundView");
            };
...
var BlackCat;
(function (BlackCat) {
    class RefundView extends BlackCat.ViewBase {
        constructor() {
            super(...arguments);
            this.Balances = {
                gas: "0",
        ...
        start() {
            ...
            if (BlackCat.PayTransferView.transferType_default) {
                this.transferType = BlackCat.PayTransferView.transferType_default;
            }
            else {
                this.transferType = "GAS";
            }
        ...
        create() {
            ...
            var popupTitle = this.objCreate('div');
            popupTitle.classList.add("pc_popup_title");
            popupTitle.innerText = BlackCat.Main.langMgr.get("refund_transfer");
            this.ObjAppend(popupbox, popupTitle);
            var divtransfer = this.objCreate("div");
            divtransfer.classList.add("pc_transferbox");
            this.ObjAppend(popupbox, divtransfer);
            ...
            this.inputTransferAddr = this.objCreate("input");
            this.inputTransferAddr.classList.add("pc_transaddress");
            this.inputTransferAddr.placeholder = BlackCat.Main.langMgr.get("refund_address");
            this.inputTransferAddr.value = this.getAddress();
            this.inputTransferAddr.onfocus = () => {
                this.inputTransferAddr.select();
            };
            ...
            this.ObjAppend(divtransfer, divTransferCount);
            this.inputTransferCount = this.objCreate("input");
            this.inputTransferCount.placeholder = BlackCat.Main.langMgr.get("refund_amount");
            this.ObjAppend(divTransferCount, this.inputTransferCount);
            this.inputTransferCount.onkeyup = () => {
                this.doinputchange();
            };
            ...
            this.divHaveBalanceAmounts.textContent = "";
            this.ObjAppend(this.divHaveAmounts, this.divHaveBalanceAmounts);
            this.netFeeCom = new BlackCat.NetFeeNoSlide(popupbox, (net_fee) => {
                this.netFeeChange(net_fee);
            });
        ...
        toRefer() {
            if (RefundView.refer) {
                BlackCat.Main.viewMgr.change(RefundView.refer);
                RefundView.refer = null;
            }
        }
        key_esc() {
        }
        getAddress() {
            if (RefundView.address) {
                this.toaddress = RefundView.address;
                RefundView.address = "";
            }
            return this.toaddress;
        }
        getAddressbalance() {
            this.inputTransferCount.value = parseInt(this.Balances.neo).toString();
        }
        doinputchange() {
            if (this.transferType == "NEO") {
                var neo_int = parseInt(this.inputTransferCount.value);
        ...
        gatSelect() {
            this.divTransferAddr.classList.add("pc_transfer_active");
            this.labelName.textContent = BlackCat.PayTransferView.contact.address_name + ":";
            this.inputTransferAddr.value = BlackCat.PayTransferView.contact.address_wallet;
            var labelNameW = this.labelName.clientWidth + 10;
            var inputTransferAddrw = labelNameW + 35;
            this.inputTransferAddr.style.width = "calc( 100% - " + inputTransferAddrw + "px";
            this.inputTransferAddr.style.padding = "0 35px 0 " + labelNameW + "px";
        ...
        doTransfer() {
            return __awaiter(this, void 0, void 0, function* () {
                ...
                switch (this.transferType) {
                    ...
                    case "CNEO":
                    case "CGAS":
                        if (Number(net_fee) > Number(this.Balances.neo)) {
                            BlackCat.Main.showErrMsg("pay_transferGasNotEnough", () => {
                                this.inputTransferCount.focus();
                            });
                            return;
                ...
                try {
                    ...
                    res.err = true;
                    res.info = e.toString();
                    console.log("[BlaCat]", '[RefundView]', 'doTransfer, tools.CoinTool.rawTransaction error => ', e.toString());
                }
                BlackCat.Main.viewMgr.viewLoading.remove();
                if (res) {
                    console.log("[BlaCat]", '[RefundView]', '转账结果 => ', res);
                    if (res.err == false) {
                        yield BlackCat.ApiTool.addUserWalletLogs(BlackCat.Main.user.info.uid, BlackCat.Main.user.info.token, res.info, "0", this.inputTransferCount.value, "6", '{"sbPushString":"transfer", "toaddr":"' + this.toaddress + '", "count": "' + this.inputTransferCount.value + '", "nnc": "' + BlackCat.tools.CoinTool["id_" + this.transferType] + '"}', BlackCat.Main.netMgr.type, "0", net_fee, RefundView.log_type_detail[this.transferType.toLowerCase()]);
                        BlackCat.Main.showInfo(("pay_transferDoSucc"));
                        this.remove();
                        if (RefundView.callback)
                            RefundView.callback();
                        RefundView.callback = null;
                    }
                    else {
                        BlackCat.Main.showErrMsg(("pay_transferDoFail"));
                    }
    ...
    RefundView.address = "";
    BlackCat.RefundView = RefundView;
})(BlackCat || (BlackCat = {}));
var BlackCat;
(function (BlackCat) {
    ...
    class ViewMgr {
        ...
        change(type) {
            switch (type) {
                ...
                case "RefundView":
                    if (!this.RefundView) {
                        this.RefundView = new BlackCat.RefundView();
